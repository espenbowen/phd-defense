...
\begin{frame}{The \lhcb experiment}
  ...
  \begin{columns}
    ...
    \begin{column}{0.36\textwidth}
      \centering
      \begin{tikzpicture}
        \node[anchor=south west,inner sep=0] at (0,0) {\includegraphics[height=3.5cm]{figs/lhcb/bsmix.pdf}};
...
\begin{column}{0.7\textwidth}
   \begin{itemize}
     \item Plays important role in selecting interesting events and reducing the data rate to disk 
   \end{itemize}
\vspace{-0.3cm}
...
%      \item Two stages (HLT1 and HLT2)
      \item Track reconstruction and PV finding performed
        \begin{itemize}
        \item[\ding{70}] Simplified reconstruction w.r.t offline (IP cuts applied)
...
%       \item Achieve offline-quality alignment and calibration
%     \end{enumerate}
% \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      \begin{itemize}
        ...
        \item[\ding{80}] Existing tracking sequence unable to meet timing budget!
      \end{itemize}
    \end{enumerate}
  \end{itemize}
...
\begin{myenv}{Efficiency too low/falls off with increasing \boldmath{\ptot}}[linecolor=barcolor]
  \begin{itemize}
    \setlength{\itemindent}{0.5em}
...
\begin{itemize}
  \item[$\blacktriangleright$] Large LHCb datasets allow for precise measurements of the $\deriv\BF/\deriv\qsq$ of \btosll processes
...
\begin{tikzpicture}
    \centering
    \node[anchor=south west,inner sep=0](image) at (0,0) {\includegraphics[width=0.45\textwidth]{figs/kpimm/introduction/AFB.pdf}};
    \begin{scope}[x={(image.south east)},y={(image.north west)}]
      %\draw[help lines,xstep=.1,ystep=.1] (0,0) grid (1,1);
...
%\begin{itemize}
  ...
  %\item $P'_{5}$ anomaly recently confirmed by the Belle collaboration
%\end{itemize}
...
%  \begin{scope}[x={(image.south east)},y={(image.north west)}]
%   %\draw[help lines,xstep=.1,ystep=.1] (0,0) grid (1,1);
%   \node[draw=none] at (0.55,0.97) {\tiny 1330\textless\mkpi\textless1530\mevcc};
%  \end{scope}
% \end{tikzpicture}
\end{frame}
...
\node[draw=none] at (1.6,4.5) {\tiny \BdToJPsiKpi};
\end{tikzpicture} 
};
