...
\begin{center}
\includegraphics[height=0.45\textheight]{figs/matter-antimatter.jpg}
\includegraphics[height=0.45\textheight]{figs/darkmatter.jpg}
\includegraphics[height=0.45\textheight]{figs/13_oscill.jpeg}
\end{center}
...
\begin{itemize}
  ...
  \item In the SM neutrinos are only left-handed and massless, but we know they have a small but non-zero mass
\end{itemize}
\end{frame}
...
\begin{itemize}
\item These limitations have led physicists to look for extensions to the SM in the form of New Physics (NP) models
\end{itemize}
...
\item Can search for these NP particles through {\bf direct} and {\bf indirect} approaches
   \begin{itemize}
     \item Direct \to produce new particles `on-shell'
     \item Indirect \to study the effect of `off-shell' (virtual) particles within quantum loop
   \end{itemize}
\end{itemize}
...
\begin{itemize}
\item Rare decays such as \Bsmm proceed via loop level diagrams
...
\item As flavour changing neutral currents such as \Bsmm are suppressed in the SM, NP can contribute at the same level and change the decay properties
%\begin{itemize}
...
\begin{itemize}
 \item For \lhcb Run I, this consisted of two stages:
 \begin{enumerate}
...
\begin{itemize}
  \item Three important figures of merit used to evaluate the performance of tracking algorithms are:
\end{itemize}
\begin{itemize}
...
\end{itemize}
\begin{itemize}
  \item[\ding{184}] Execution time per event
\end{itemize}
...
\begin{frame}{Reminder: Rare decays as a probe for NP}
  \begin{columns}
    \begin{column}{0.6\textwidth}
      \begin{itemize}
      \item Rare flavour-changing-neutral-current processes such as \btosll are only possible via loop diagrams in SM
        \begin{itemize}
...
\begin{itemize}
 \item Decay fully described by \qsq, and three angles $\Omega=(\ctl,\ctk,\phi)$
 \end{itemize}
  ...
  \begin{itemize}
  \item $\textcolor{cobalt}{{F_{\rm L}}}$, $\textcolor{cobalt}{{A_{\rm FB}}}$, $\textcolor{cobalt}{{S_i}}$ are observables which are measured
...
\begin{itemize}
  \item[\ding{80}] Results for a {\bf consistent pattern} with the discrepancies in $\deriv\BF/\deriv\qsq$ measurements
  \item[\ding{80}] Possible contribution of NP, e.g. a new Z' boson
  \item[\ding{80}] Another hypothesis is the underestimation of the QCD (charm loop) effects 
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Analyses of \BdToKpimm at \lhcb have focused on the (P-wave) $K^\ast_{1}(892)^0$
\item Region of \mkpi $\sim$1430\mevcc contains contributions from $K^\ast$ states in \mbox{S$-$, P$-$ and D$-$wave} configurations
\item Allows for a complementary measurement of \btosll transitions
\item Have performed a measurement of the differential branching fraction and angular analysis in this new \mkpi region
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    %\item[\ding{70}] Scaling factor to account for different \mkpi, \qsq determined from simulation
  \end{itemize}
...
\begin{itemize}
\item \CP-averaged differential decay rate of \BdToKpimm decays with the \Kp\pim system in a S-, P- or D-wave configuration is expanded in an orthonormal basis of angular functions, $f_i(\Omega)$, %{\scriptsize \bf \href{http://journals.aps.org/prd/abstract/10.1103/PhysRevD.92.033013}{[PRD92(2015)033013]}}
...
% \item Signal region: $5230<\mkpimm<5330$~\mevcc
% \item Background region: $5350<\mkpimm<5700$~\mevcc
\end{itemize}
...
\begin{itemize}
  %\item Full set of moments and $40\times40$ covariance matrix available on HepData portal
