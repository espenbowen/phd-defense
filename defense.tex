...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item The Standard Model is the cornerstone of modern particle physics
\item[]
\item It describes our current understanding of elementary particles and their interactions
\item[]
...
\begin{itemize}
  \item The observed asymmetry between matter and antimatter in our Universe cannot be explained
  \item Dark matter, which is believed to be 5 times as abundant as ordinary matter, is not accounted for
  \item SM neutrinos are only left-handed and massless, but we know they have a small, non-zero mass
\end{itemize}
\end{frame}
...
\item In NP models, new particles would also appear in the loop
\item As decays such as \Bsmm are suppressed in the SM, NP can contribute at the same level and alter the properties of the decay in a measurable way
\end{itemize}
...
\begin{frame}{The \lhcb experiment}
  \begin{itemize}
  \item LHCb is a particle physics detector experiment located at the LHC 
  \item Its primary goal is to search for {\bf indirect} evidence of New Physics in the decays of \bquark and \cquark quarks
...
\begin{itemize}
  \item The data rate from every \proton\proton collision is too high store to every event
  \begin{itemize}
    \item 10 exobytes per year!
  \end{itemize}
  \item Moreover, many of the collisions are not of interest for physics analyses
  \item The \lhcb trigger plays an important role in selecting interesting events and reducing the data rate to disk 
\end{itemize}
...
\begin{column}{0.6\textwidth}
  \begin{itemize}
    \item \lhcb  will undergo an upgrade during 2018-2019 to allow data taking at higher energy and intensity
...
\begin{frame}\frametitle{Upstream tracking (VeloTT) in Run 1}
  ...
  \item[\ding{80}] Fit track with a Kalman filter (slow)
  \end{itemize}
...
\begin{itemize}
  \item Initial version of VeloUT was a replication of the VeloTT algorithm from Run 1
\end{itemize}

\begin{itemize}
  \item Execution time of 27.20\ms is {\bf far too slow} to use in the software trigger
  \begin{itemize}
    \item[\ding{70}] c.f. VELO tracking $\sim1.8\ms$
  \end{itemize}
  \item Reconstruction efficiency is {\bf too low}
  \begin{itemize}
...
\begin{itemize}
\item[\ding{80}] \lhcb will become the first hadron collider experiment to operate a software-only trigger
\end{itemize}
...
\begin{frame}{Reminder: Rare decays as a probe for NP}
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      \begin{itemize}
        \item e.g. enhance/suppress branching fractions or alter angular distributions%, new sources of \CP violation
      \end{itemize}
      \item[\ding{70}] Can compare measurements to SM predictions
      \end{itemize}
    \end{column}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Measured as a function of $\qsq\equiv m_{\mu\mu}^{2}$
  \end{itemize}
  ...
  \item[\ding{80}] Results point towards lower rates than predicted by the SM
\end{itemize}
...
\item Can write an expression for angular decay distribution 
  \begin{itemize}
...
\end{itemize}
  ...
  \begin{itemize}
    ...
    \end{itemize}
  \item Often useful to measure ratios of observables e.g. $P'_{4,5} = \textcolor{cobalt}{S_{4,5}}/\sqrt{ \textcolor{cobalt}{F_{\rm L}}(1 -  \textcolor{cobalt}{F_{\rm L}})}$
%    \begin{itemize}
%    \item[\ding{70}] 
%    \end{itemize}
  \end{itemize}
...
\begin{itemize}
    \item[\ding{80}] Deviations in $P'_{5}$ at level of 2.9$\sigma$ in two \qsq bins
\end{itemize}
...
\begin{itemize}
  \item Fit to invariant mass distribution required to separate the signal from background contributions
...
\begin{itemize}
  \item The selection efficiency of the signal candidates varies as a function of the decay angles $\thetal,\thetak,\phi$
  \item In order to perform an angular analysis, need to carefully understand this effect
  \item In this analysis, the efficiency distribution is modelled using a Legendre polynomial parameterisation
  %\begin{itemize}
...
\begin{itemize}
\item[\ding{80}] Hope this measurement stimulates further theory effort in this area
\end{itemize}
\end{itemize}
...
\item Define 40 normalised angular moments which form the set of observables that are measured
\end{itemize}
\begin{equation*}
...
\end{itemize}
\begin{itemize}
  \item Results point towards large interference between S- and P-wave states 
  \item Hints of suppressed D-wave contribution% than expected
%  \begin{itemize}
...
\begin{itemize}
  \item[\ding{72}] Novel idea to use upstream tracking to speed up the Upgrade software trigger was a success!
  \item[\ding{72}] New algorithm adopted into the default tracking sequence
  \item[\ding{80}] First measurements of the differential branching fraction and angular analysis of \BdToKpimm in \Kstarfourteenthirty region
