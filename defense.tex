...
\begin{myenv}{Efficiency too low/falls off with increasing \ptot}[linecolor=barcolor]
  \begin{itemize}
    ...
    \item Due to assumptions made in cluster
    \item New hit clustering sequence 
  \end{itemize}
\end{myenv}
...
\begin{myenv}{Execution time much too slow for use in trigger}[linecolor=barcolor]
  \begin{itemize}
    \setlength{\itemindent}{1em}
    \item Replace `detector regions' with hit sorting and binary searches
    \item Remove Kalman filter, simple $\chi^{2}$ fit is sufficient
    \item Numerous \cpp optimisations/changes to the logic
  \end{itemize}
\end{myenv}
