...
\begin{itemize}
\item Track reconstruction in \lhcb is performed by several different algorithms
...
\item Long tracks used for majority of \lhcb analyses
  \begin{itemize}
    ...
    \item[\ding{70}] Best momentum resolution
  \end{itemize}
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
%Slide
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{The \lhcb Upgrade}

\begin{columns}
\begin{column}{0.3\textwidth}
\includegraphics[width=\textwidth]{figs/detector/trigger-run3.pdf}
\end{column}
\begin{column}{0.7\textwidth}
  \begin{itemize}
    \item 
...
\begin{frame}\frametitle{Upstream tracking (VeloTT) in Run 1}
  ...
  \item[\ding{72}] Fit track with full Kalman filter
  \end{itemize}
...
\begin{frame}{Rare decays as a probe for New Physics}
  \begin{columns}
    ...
    \begin{myenv}{\color{bostonuniversityred}{NP}}[linecolor=bostonuniversityred]
      ...
      \node[draw=none,bostonuniversityred] at (0.5,0.47) {\small ?};
      \end{scope}
      \end{tikzpicture}
    \begin{tikzpicture}
...
\begin{itemize}
  ...
  \begin{itemize}
  \item All degrees of freedom above a given energy scale are integrated out
  \item Introduce set of Wilson coefficients, $\mathcal{C}_{i}$, and local operators, $\mathcal{O}_{i}$, encoding coupling strength and Lorentz structure
  \end{itemize}
\end{itemize}
\begin{equation*}
...
%Slide
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{\BdToKpimm in the \Kstarfourteenthirty region}
\begin{itemize}
...
\includegraphics[width=0.45\paperwidth]{figs/detector/gluon_fusion.eps}
\includegraphics[width=0.45\paperwidth]{figs/detector/quark_antiquark_annihilation.eps}
...
\begin{frame}\frametitle{}
  ...
  \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}
      ...
      \node[draw=none] at (0.2,0.8) {p};
      \node[draw=none] at (6.8,0.8) {p};
    ...
    \begin{columns}
      \begin{column}{0.49\textwidth}
        \begin{myenv}{Beauty signatures}[linecolor=barcolor]
          \begin{itemize}
          \item B$^{\pm}$ mass 5.28 GeV, daughter $p_{T}$ \orderof(1 GeV)
          \item $\tau \sim $1.6 ps $\to$ L $\sim$ 1 cm $\to$ large IP
      ...
      \begin{column}{0.49\textwidth}
        \begin{myenv}{Charm signatures}[linecolor=barcolor]
          \begin{itemize}
          \item D$^{0}$ mass 1.86 GeV, large daughter $p_{T}$ 
          \item $\tau \sim $0.4 ps $\to$ L $\sim$ 0.4 cm $\to$ smaller IP
          %\item Can also come from `secondary' B decays
          \end{itemize}
        \end{myenv}
      \end{column}
    \end{columns}
...
\medskip
\centering
\includegraphics[width=0.5\textwidth]{figs/upstream-tracking-upgrade/wrong_side_hits.pdf}
\end{frame}
